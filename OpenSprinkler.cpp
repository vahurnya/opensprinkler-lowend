...
ulong OpenSprinkler::sensor_measurement_suspend_until;
...
void OpenSprinkler::begin()
{
	...
	if (detect_i2c(ACDR_I2CADDR))
	{ // check if main PCF8574 exists [ip] In my case there is no MAIN_I2CADDR but an ACDR_I2CADDR (8 bits on the extender address)
		...
		PIN_PWR_SENSOR1 = V0_PIN_PWR_SENSOR1;
	...
	pinModeExt(PIN_PWR_SENSOR1, OUTPUT);
...
void OpenSprinkler::apply_all_station_bits()
{
	...
	if (iopts[IOPT_SPE_AUTO_REFRESH])
	{
		...
		if (sid != last_sid)
		{ // avoid refreshing the same station twice in a roll
			last_sid = sid;
			bid = sid >> 3;
			s = sid & 0x07;
...
{
	if (sensor_measurement_suspend_until == 0)
	{
		// during boot the 1st measurement timing must be configured *IP
		sensor_measurement_suspend_until = curr_time;
	}
	if (curr_time >= sensor_measurement_suspend_until)
	{
		if (curr_time < (sensor_measurement_suspend_until + 3))
		{
			// Provide max 15mA power to rain sensor for the upcoming measurement
			digitalWriteExt(PIN_PWR_SENSOR1, HIGH);
		}
		else
		{
			// configure the next measurement time (300 seconds)
			sensor_measurement_suspend_until = curr_time + 60;
...
#endif
			digitalWriteExt(PIN_PWR_SENSOR1, LOW);
		}
	}
}
...
{
	sensor_measurement_suspend_until = 0;
	sensor1_on_timer = 0;
	sensor1_off_timer = 0;
	sensor2_off_timer = 0;
...
void OpenSprinkler::options_setup()
{
	...
	if (file_read_byte(IOPTS_FILENAME, IOPT_FW_VERSION) < 219 || // fw version is invalid (<219)
		...
		file_read_byte(IOPTS_FILENAME, IOPT_RESET) == 0xAA)
	{ // reset flag is on
...
void OpenSprinkler::detect_expanders()
{
	for (byte i = 0; i < (MAX_NUM_BOARDS) / 2; i++)
	{
		...
		else if (type == IOEXP_TYPE_8574)
		{ // [ip] In my case the expander is a PCF8574
